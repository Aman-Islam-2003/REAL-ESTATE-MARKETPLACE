      ...
      <form className='flex flex-col sm:flex-row gap-4' onSubmit={handleSubmit} >
        ...
        <div className='flex flex-col flex-1 gap-4' >
          <p className='font-semibold'>Images:
            <span className='text-gray-600 font-normal ml-2'>The first image will be the cover (max 6)</span>
          </p>
          <div className='flex  gap-4'>
            ...
            <button type='button' disabled={uploading} className='border p-3 uppercase rounded-lg border-green-700 text-green-700 hover:bg-green-700 hover:text-white' onClick={submitImageHandler}>{uploading ? 'Uploading...' : 'Upload'}</button>
          </div>
          <p className='text-red-700'>{imageUploadError ? imageUploadError : ''}</p>
          {formData.imageUrls.length > 0 &&
            formData.imageUrls.map((url, index) => (
              <div key={index} className='flex border justify-between rounded p-3 items-center'>
                <img
                  src={url}
                  alt='listing image'
                ...
                <button
                  type='button'
                  className='p-3 text-red-700 uppercase hover:opacity-75' onClick={() => imageDeleteHandler(index)}
                >
                  Delete
                </button>
              </div>
            ))}
          <button className='p-3 bg-slate-700 text-white uppercase rounded-lg disabled:opacity-800 hover:opacity-95' disabled={loading || uploading}>{loading ? 'Loading....' : "Create Listing"}</button>
          {
